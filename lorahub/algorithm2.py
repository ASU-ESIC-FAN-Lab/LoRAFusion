    ...
    for i, peft_model_id in enumerate(lora_module_list):
        ...
        if i == 0:
            for j,key in enumerate(keys):
                if 'encoder' in key:
                    final_state_dict[key] = weights[i][0] * lora_state_dict[key]
                else:
                    final_state_dict[key] = weights[i][1] * lora_state_dict[key]
        else:
            for j,key in enumerate(keys):
                if 'encoder' in key:
...
def lorahub_learning(lora_module_list: List[str], 
    ...
    num_blocks=len(cache[lora_module_list[0]].keys())//12
    num_blocks = 2
    instrum = ng.p.Array(shape=(number_of_loras,num_blocks),upper=1.5,lower=-1.5)
    # instrum = ng.p.Array(
    ...
    print(recommendation.value.flatten())
    #save value to txt
    with open('recommendation.txt', 'w') as f:
        for item in recommendation.value:
            f.write("%s\n" % item)
