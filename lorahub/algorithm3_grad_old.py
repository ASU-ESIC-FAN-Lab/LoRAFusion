...
def default_get_loss(example_dataset, model, batch_size, compute_gradients=True):
    """
    ...
    for _, batch in enumerate(train_dataloader):
        ...
        if compute_gradients:
            ...
            # Compute loss without gradients (evaluation)
            with torch.no_grad():
                outputs = model(**batch)  # Forward pass through the model
                loss = outputs.loss
                train_loss += loss.detach().float()  # Accumulate the loss
    
    # Return the average loss over the number of examples
    if compute_gradients:
        return train_loss / len(example_dataset["input"])  # Keep as tensor for backpropagation
    else:
        return float(train_loss) / len(example_dataset["input"]) 
...
def get_score(weights, model, cache, example_dataset, batch_size, get_loss, get_regular):
    ...
    # # print("length of lor modules:",len(lora_module_list))
    # # print(weights)
    ...
    #     if name in final_state_dict:
    #         param.requires_grad = True
    #         param.copy_(final_state_dict[name])

    # for k,v in final_state_dict.items():
    #     if v.requires_grad:
    #         print(f"{k} requires gradient.")
    #     else:
    #         print(f"{k} does not require gradient.")
    # for name, param in model.named_parameters():
...
def lorahub_learning(lora_module_list: List[str], 
    ...
    # standard
    # params = Variable(torch.rand(number_of_loras, num_blocks ),requires_grad=True)
    # xavier]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # params_ = Variable(torch.empty(number_of_loras, num_blocks), requires_grad=True)
    ...
    #         # if params[i][j].grad is None:
    #             params.grad[i,j] = torch.zeros_like(params[i][j])
    #             print(params.grad[i,j])

    optimizer = optim.Adam([params], lr=lr) #weight decay
    ...
    for step in range(max_inference_step):
        optimizer.zero_grad()
        loss = get_score_partial(params)
        #print gradient of  base_model.model.encoder.block.0.layer.0.SelfAttention.q.lora_A.default.weight
        loss.backward()
        for name, param in model.named_parameters():
            name_processed = name.replace(".default","")
            if name_processed in final_state_dict:
                # final_state_dict[name_processed]
                # print(param.grad.shape)
                # print(final_state_dict[name_processed].grad.shape)
                for i,j,peft_model_id in key_params_lookup[name_processed]:
                    # print(param.grad,cache[peft_model_id][name_processed])
                    params.grad[i,j] += (param.grad * cache[peft_model_id][name_processed]).sum()
                    # result=(param.grad * cache[peft_model_id][name_processed]).sum()
                    # print(params[i][j].grad)
        # print(params[0][0])
        optimizer.step()
        # print(params[0][0])
        update_lora()
